...
class MainWindow(ctk.CTk):
    def __init__(self):
        ...
        try:
            ...
            btn_logo = ctk.CTkButton(self.menu_frame, image=logo, text="", width=80, height=50, fg_color="transparent")
            btn_logo.pack(pady=12)
        except Exception:
            ctk.CTkLabel(self.menu_frame, text="HEALTH TRACK", font=("Arial",16,"bold")).pack(pady=12)

        ctk.CTkLabel(self.menu_frame, text="MENU", text_color="#0B395C", font=("Arial",18,"bold")).pack(pady=(6,16))

        # botões do menu
        self.btn_home = ctk.CTkButton(self.menu_frame, text="Home", command=lambda: self.show_screen("home"))
        self.btn_home.pack(pady=6, padx=20, fill="x")

        self.btn_lista = ctk.CTkButton(self.menu_frame, text="Pacientes", command=lambda: self.show_screen("lista"))
        self.btn_lista.pack(pady=6, padx=20, fill="x")

        # atalhos
        ctk.CTkLabel(self.menu_frame, text="ATALHOS", text_color="#0B395C", font=("Arial",14,"bold")).pack(pady=(20,6))
        self.btn_cadastrar = ctk.CTkButton(self.menu_frame, text="Cadastrar Paciente", command=lambda: self.show_screen("cadastro"))
        self.btn_cadastrar.pack(pady=6, padx=20, fill="x")

        self.btn_editar = ctk.CTkButton(self.menu_frame, text="Editar Paciente", command=lambda: self.show_screen("editar"))
        self.btn_editar.pack(pady=6, padx=20, fill="x")

        self.btn_excluir = ctk.CTkButton(self.menu_frame, text="Excluir Paciente", command=lambda: self.show_screen("excluir"))
        ...
        self.btn_sair = ctk.CTkButton(self.menu_frame, text="Sair", fg_color="#d9534f", hover_color="#c9302c", command=self.destroy)
